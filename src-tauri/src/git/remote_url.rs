...
mod tests {
    ...
    fn enterprise_subdomains_keep_their_provider() {
        assert_eq!(
            p("git@github.myco.com:o/r.git").provider,
            RemoteProvider::GitHub
        );
        assert_eq!(
            p("https://gitlab.myco.com/o/r.git").provider,
            RemoteProvider::GitLab
        );
        assert_eq!(
            p("https://github.myco.com/o/r").web_base(),
            "https://github.myco.com/o/r"
        );
    }
    ...
    fn unrelated_hosts_are_self_hosted_not_misdetected() {
        assert_eq!(
            p("https://git.example.com/o/r.git").provider,
            RemoteProvider::SelfHosted
        );
        // "github" appearing in the path must not classify the host.
        assert_eq!(
            p("https://git.example.com/github/r.git").provider,
            RemoteProvider::SelfHosted
        );
    }
    ...
    fn recognizes_the_other_major_hosts() {
        assert_eq!(
            p("git@bitbucket.org:o/r.git").provider,
            RemoteProvider::Bitbucket
        );
        assert_eq!(
            p("https://codeberg.org/o/r.git").provider,
            RemoteProvider::Gitea
        );
        assert_eq!(
            p("https://git.sr.ht/~user/repo").provider,
            RemoteProvider::SourceHut
        );
        assert_eq!(
    ...
    fn branch_names_with_slashes_and_spaces_are_encoded_per_segment() {
        ...
        assert_eq!(
            r.branch_url("feature/a b").unwrap(),
            "https://github.com/o/r/tree/feature/a%20b"
        );
    }
