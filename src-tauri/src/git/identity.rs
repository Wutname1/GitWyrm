...
pub fn write_identity(name: &str, email: &str) -> Result<(), AppError> {
  ...
  if name.is_empty() {
    return Err(AppError::Other("Enter the name to put on your commits.".into()));
  }
  if email.is_empty() {
    return Err(AppError::Other("Enter the email to put on your commits.".into()));
  }
...
mod tests {
  ...
  fn an_identity_needs_both_halves() {
    ...
    for partial in [
      GitIdentity { name: "Ada".into(), email: String::new() },
      GitIdentity { name: String::new(), email: "ada@example.com".into() },
      GitIdentity::default(),
    ] {
