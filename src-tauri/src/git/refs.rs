    ...
    for (kind, is_remote) in [(BranchType::Local, false), (BranchType::Remote, true)] {
        for (branch, _) in repo.branches(Some(kind))?.flatten() {
            ...
            let is_head = !is_remote
                && (branch.is_head()
                    || (tip.is_some() && tip == head_oid && Some(&name) == head_name.as_ref()));
...
mod tests {
    ...
    fn skips_symbolic_remote_head() {
        ...
        s.repo
            .reference("refs/remotes/origin/main", tip, true, "")
            .unwrap();
        s.repo
            .reference_symbolic(
    ...
    fn remote_only_branch_is_enumerated_with_a_tip() {
        ...
        s.repo
            .reference("refs/remotes/origin/claude/feature", ahead, true, "")
            .unwrap();
        ...
        assert_eq!(
            rec.short_name(),
            "claude/feature",
            "remote prefix is stripped once"
        );
    ...
    fn remote_ahead_of_head_reports_positive_ahead() {
        ...
        s.repo
            .reference("refs/remotes/origin/feature", ahead, true, "")
            .unwrap();
    ...
    fn local_branch_is_flagged_as_head() {
        ...
        assert_eq!(
            head.short_name(),
            head.name,
            "locals are not prefix-stripped"
        );
    }
}
