...
pub fn apply_to_repo(repo_path: &str, profile: &Profile) -> Result<(), AppError> {
    ...
    match &profile.signing {
        ...
        _ => {
            ...
            let _ = run_git(
                Some(repo_path),
                &["config", "--local", "--unset", "user.signingkey"],
            );
...
mod tests {
    ...
    fn include_rules_are_case_insensitive() {
        ...
        assert!(
            block.contains("path = C:/cfg/profile-p1.gitconfig"),
            "got: {block}"
        );
    }
