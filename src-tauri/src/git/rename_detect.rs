...
mod tests {
    ...
    fn small_rename_is_still_detected() {
        ...
        index
            .add_path(std::path::Path::new("before.txt"))
            .expect("add");
        ...
        index
            ...
            .expect("remove");
        index
            .add_path(std::path::Path::new("after.txt"))
            .expect("add");
        index.write().expect("write index");
        let tree_id = index.write_tree().expect("tree");
        ...
        assert_eq!(
            diff.deltas().len(),
            2,
            "before detection: a delete plus an add"
        );
        ...
        assert_eq!(
            diff.deltas().len(),
            1,
            "a rename under the cap must collapse to one delta"
        );
    }
    ...
    fn commit_many(
        ...
        repo.commit(Some("HEAD"), &sig, &sig, "bulk", &tree, &refs)
            .expect("commit")
    }
    ...
    fn a_huge_diff_skips_rename_detection() {
        ...
        assert!(
            before > MAX_DELTAS_FOR_RENAMES,
            "fixture must exceed the gate"
        );
