...
fn chunks(name: &str) -> Vec<Chunk<'_>> {
  ...
  while i < bytes.len() {
    ...
    out.push(if digit { Chunk::Num(slice) } else { Chunk::Text(slice) });
  }
  out
}
...
pub fn compare_tag_names(a: &str, b: &str) -> Ordering {
  ...
  loop {
    match (ia.next(), ib.next()) {
      ...
      (None, Some(_)) => {
        return if has_prerelease(b) { Ordering::Greater } else { Ordering::Less }
      }
      (Some(_), None) => {
        return if has_prerelease(a) { Ordering::Less } else { Ordering::Greater }
      }
      (Some(x), Some(y)) => {
        let ord = match (x, y) {
...
mod tests {
  ...
  fn the_reported_tag_list_orders_newest_first() {
    let got = sorted_desc(&[
      "0.0.9", "0.0.8", "0.0.7", "0.0.6", "0.0.5", "0.0.4", "0.0.3", "0.0.2-R2", "0.0.2-Beta1",
      "0.0.2", "0.0.11", "0.0.10",
    ]);
    assert_eq!(
      got,
  ...
  fn v_prefixed_and_dated_tags_order_sensibly() {
    assert_eq!(sorted_desc(&["v1.2.0", "v1.10.0", "v1.9.0"]), vec!["v1.10.0", "v1.9.0", "v1.2.0"]);
    assert_eq!(
      sorted_desc(&["release-2026-01-05", "release-2026-01-20", "release-2025-12-31"]),
      vec!["release-2026-01-20", "release-2026-01-05", "release-2025-12-31"]
    );
  }

  #[test]
  fn non_version_tags_keep_alphabetical_order() {
    assert_eq!(sorted_desc(&["alpha", "beta", "gamma"]), vec!["gamma", "beta", "alpha"]);
  }
