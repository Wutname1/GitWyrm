...
pub fn list(repo: &git2::Repository, current_path: Option<&Path>) -> Vec<Worktree> {
  ...
  linked.sort_by(|a, b| a.folder_name.to_lowercase().cmp(&b.folder_name.to_lowercase()));
  out.extend(linked);
  out
}
...
pub fn suggest_path(main_workdir: &Path, branch: &str) -> String {
  ...
  let parent = main_workdir.parent().map(Path::to_path_buf).unwrap_or_else(|| {
    main_workdir.to_path_buf()
  });
...
pub fn path_problem(main_workdir: &Path, path: &Path) -> Option<String> {
  ...
  if path.exists() {
    if !path.is_dir() {
      return Some("That path is a file, not a folder. Pick a folder name that does not exist yet.".into());
    }
    let empty = std::fs::read_dir(path).map(|mut d| d.next().is_none()).unwrap_or(false);
    if !empty {
      return Some(
        "That folder already has files in it. Pick an empty folder or a new folder name.".into(),
      );
    }
  }
...
pub fn add(
  ...
  let branch = branch.trim();
  if branch.is_empty() {
    return Err(AppError::Other("Choose a branch for the new worktree.".into()));
  }
...
mod tests {
  ...
  fn dirty_count_clean_only_when_both_zero() {
    assert!(DirtyCount { modified: 0, untracked: 0 }.is_clean());
    assert!(!DirtyCount { modified: 1, untracked: 0 }.is_clean());
    assert!(!DirtyCount { modified: 0, untracked: 1 }.is_clean());
  }
