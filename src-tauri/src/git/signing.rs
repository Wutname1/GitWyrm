...
fn migrate_legacy_gnupg_home() {
    ...
    match std::fs::rename(&legacy, &current) {
        ...
        Err(e) => log::warn!("could not move signing keys from {}: {e}", legacy.display()),
    }
}
...
fn gpgsign_scope(repo_path: &str) -> Option<String> {
    for (scope, label) in [
        ...
        let found = run_git(
            Some(repo_path),
            &["config", scope, "--get", "commit.gpgsign"],
        )
...
pub fn forget_key_if_configured(repo_path: &str, key_id: &str) -> Result<(), AppError> {
    ...
    let points_at_it = configured.as_deref().is_some_and(|c| {
        c.eq_ignore_ascii_case(key_id) || key_id.ends_with(c) || c.ends_with(key_id)
    });
...
pub fn repair_gpg_format(repo_path: &str) -> Result<(), AppError> {
    for scope in ["--local", "--global", "--system"] {
        ...
        let _ = run_git(
            Some(repo_path),
            &["config", scope, "--unset-all", "gpg.format"],
        );
    }
...
fpr:::::::::FEDCBA9876543210FEDCBA9876543210FEDCBA98:";
    ...
    fn clearing_leaves_a_real_key_alone() {
        ...
        assert_eq!(
            repo.local("user.signingkey").as_deref(),
            Some("1C7195BAAB12CF6A")
        );
        assert!(!has_blank_signing_key(repo.path()));
    }
    ...
    fn signing_scope_names_the_repository_when_set_locally() {
        ...
        assert_eq!(
            gpgsign_scope(repo.path()).as_deref(),
            Some("this repository")
        );
    }
