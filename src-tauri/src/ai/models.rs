...
fn dedupe_by_id(models: Vec<CatalogModel>) -> Vec<CatalogModel> {
    ...
    let mut by_id: std::collections::HashMap<String, CatalogModel> =
...
async fn fetch_live(
    ...
    if provider.id == super::copilot_sdk::PROVIDER_ID {
        let token = match auth::get(app, &provider.id)? {
            ...
            None => {
                log::info!(
                    "model detection: no credential configured for provider={}",
                    provider.id
                );
                return Ok(Vec::new());
            }
    ...
    log::info!(
        "model detection request: provider={}, url={}",
        provider.id,
        url
    );
