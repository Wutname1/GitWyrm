...
async fn fetch_raw(app: &tauri::AppHandle) -> Result<String, AppError> {
    ...
    let fetched = fetched.map_err(|e| {
        e.map_or_else(
            || "model catalog request never ran".to_string(),
            |e| e.to_string(),
        )
    });
...
fn parse(raw: &str) -> Result<Vec<CatalogProvider>, AppError> {
    let providers: BTreeMap<String, RawProvider> = serde_json::from_str(raw)
        .map_err(|e| AppError::Other(format!("bad model catalog: {e}")))?;
