    ...
    match provider_id {
        ...
        "anthropic" => {
            if available.has_credentials {
                ...
                Err(AgentError::NeedsReconnect {
                    detail:
                        "Claude needs an API key. GitWyrm can't use a Claude subscription for this."
                            .into(),
                })
            }
        }
        ...
        "github-copilot" => {
            if available.has_cli {
                ...
                Err(AgentError::NeedsReconnect {
                    detail:
                        "sign in to Copilot in settings, and install the Copilot command-line tool"
                            .into(),
                })
            }
        }
...
mod tests {
    ...
    const NOTHING: Available = Available {
        has_credentials: false,
        has_cli: false,
    };
    const KEY: Available = Available {
        has_credentials: true,
        has_cli: false,
    };
    const CLI: Available = Available {
        has_credentials: false,
        has_cli: true,
    };
