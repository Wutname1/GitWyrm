...
fn branch_tracking_state(repo: &git2::Repository, branch_name: Option<&str>) -> TrackingState {
    ...
    let name = match branch_name {
        ...
        None => {
            ...
            let Ok(name) = head.shorthand().map(str::to_string) else {
                return none;
            };
            name
        }
    ...
    let (ahead, behind, upstream_gone) = match (&upstream, branch.get().target()) {
        (Some(up), Some(local_oid)) => {
            let up_oid = repo
                .find_branch(up, git2::BranchType::Remote)
                .ok()
...
) -> Result<Option<PublishArgs>, AppError> {
...
fn published_count(repo: &git2::Repository, branch: &str) -> u32 {
    ...
    let Some(upstream) = local.upstream().ok().and_then(|u| u.get().target()) else {
        return 0;
    };
...
fn default_remote(repo: &git2::Repository) -> Result<String, AppError> {
    ...
    match names.len() {
        ...
        _ => names
            ...
            .cloned()
            .ok_or_else(|| {
                AppError::Other("Several remotes are set up. Pick one in Remotes first.".into())
            }),
    }
}
...
pub async fn unpushed_tags(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let on_remote: std::collections::HashSet<String> = parse_ls_remote_tags(&stdout)
            .into_iter()
            .map(|t| t.name)
            .collect();
...
mod tests {
    ...
    fn a_configured_upstream_is_tracking() {
        ...
        let head = repo
            ...
            .to_string();
        assert_eq!(
            tracking_local(&records, &format!("origin/{head}")),
            Some(head)
        );
        drop(dir);
    }
    ...
    fn tracking_follows_config_not_the_name() {
        ...
        .expect("remote ref");
        let mut local = repo
            .branch("my-work", &head_commit, true)
            .expect("local branch");
        local
            .set_upstream(Some("origin/develop"))
            .expect("upstream");
    ...
    fn track(repo: &Repository, remote: &str) {
        ...
        let mut branch = repo
            ...
            .expect("branch");
        branch
            .set_upstream(Some(&format!("{remote}/{branch_name}")))
            .expect("upstream");
    }
    ...
    fn a_tracking_branch_pushes_plainly() {
        ...
        let state = tracking_state(&repo);
        assert!(
            publish_args(&state, &repo).expect("args").is_none(),
            "no republish needed"
        );
    }
    ...
    fn a_pruned_upstream_is_republished_to_its_own_remote() {
        ...
        repo.remote("origin", "https://example.invalid/other.git")
            .expect("origin");
        track(&repo, "upstream");
        let branch_name = repo
            .head()
        ...
        repo.find_reference(&format!("refs/remotes/upstream/{branch_name}"))
            .expect("ref")
            .delete()
            .expect("prune");
        ...
        let publish = publish_args(&state, &repo)
            .expect("args")
            .expect("must republish");
        assert_eq!(
            publish.remote, "upstream",
            "goes back to the remote it named"
        );
    }
    ...
    fn a_config_naming_a_deleted_remote_falls_back() {
        ...
        assert_eq!(
            ...
            "falls back when the named remote is gone"
        );
    }
