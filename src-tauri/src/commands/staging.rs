...
fn rename_counterparts(repo: &git2::Repository, paths: &[String]) -> Vec<String> {
    ...
    for entry in statuses.iter() {
        for delta in [entry.index_to_workdir(), entry.head_to_index()]
            .into_iter()
            .flatten()
        {
            let as_str = |f: git2::DiffFile| f.path().map(|p| p.to_string_lossy().into_owned());
            let (Some(old), Some(new)) = (as_str(delta.old_file()), as_str(delta.new_file()))
            else {
                continue;
            };
            if old == new {
...
mod tests {
    ...
    fn committed_repo() -> (tempfile::TempDir, git2::Repository) {
        ...
        let tree_id = index.write_tree().expect("write tree");
        {
            ...
            let signature =
                git2::Signature::now("Test Wyrm", "test@gitwyrm.dev").expect("signature");
    ...
    fn staging_a_rename_by_new_name_also_stages_the_old_name() {
        ...
        let old_status = repo
            .status_file(Path::new(old))
            .expect("old status after unstage");
        assert!(
    ...
    fn folder_batch_actions_stay_scoped_and_discard_staged_and_unstaged_work() {
        ...
        let tracked_status = repo
            .status_file(Path::new(&tracked))
        ...
        fs::write(
            ...
            "unstaged version after staging\n",
        )
        ...
        assert_eq!(
            fs::read_to_string(dir.path().join(&tracked))
                .expect("read target")
                .trim_end(),
            "original"
        );
        ...
        assert_eq!(
            fs::read_to_string(dir.path().join("outside.txt"))
                .expect("read outside")
                .trim_end(),
            "keep this edit"
        );
        ...
        assert!(statuses
            .iter()
            .all(|entry| { !entry.path().is_ok_and(|path| path.starts_with("target/")) }));
        assert!(repo
