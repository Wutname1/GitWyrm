...
fn parse_diff(text: &str) -> Result<ParsedDiff, AppError> {
    ...
    for line in text.split_inclusive('\n') {
        ...
        if line.starts_with("@@ ") {
            seen_hunk = true;
            let (old_start, new_start) = parse_hunk_header(&line).ok_or_else(|| {
                AppError::Other(format!("unparseable hunk header: {}", line.trim_end()))
            })?;
            hunks.push(RawHunk {
                header: line,
                old_start,
    ...
    for raw in &hunk.lines {
        ...
        match sign {
            b'+' => {
                let sel = selection
                    .iter()
                    .any(|s| s.matches(hunk_index, b'+', None, Some(new_no)));
            ...
            b'-' => {
                let sel = selection
                    .iter()
                    .any(|s| s.matches(hunk_index, b'-', Some(old_no), None));
                if !sel {
...
pub(crate) fn build_patch_from_raw(
    ...
    if !any {
        return Err(AppError::Other(
            "selection produced no applicable changes".into(),
        ));
    }
    Ok(patch)
}
...
mod tests {
    ...
    fn no_newline_marker_preserved() {
        ...
        let d = parse_diff(text).unwrap();
        let out = rebuild_hunk(
            &d.hunks[0],
            0,
            &[sel(0, Some(1), None), sel(0, None, Some(1))],
            false,
        )
        .unwrap();
    ...
    fn roundtrip_stage_one_of_two_hunks() {
        ...
        let Some(dir) = scratch_repo("two_hunks", &base) else {
            return;
        };
    ...
    fn roundtrip_slidable_block_single_line() {
        ...
        let base = "alpha\n\nbeta\n\ngamma\n";
        let Some(dir) = scratch_repo("slidable", base) else {
            return;
        };
        let dpath = dir.to_string_lossy().into_owned();
        ...
        assert_eq!(
            added, 1,
            "expected exactly one added line staged, numstat: {staged_diff}"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn partial_contiguous_block_rejected() {
        ...
        assert!(
            err.is_err(),
            "addition behind an unselected deletion should be rejected"
        );
