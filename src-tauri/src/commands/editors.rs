...
impl EditorKind {
    ...
    fn commands(self) -> &'static [&'static str] {
        match self {
            ...
            EditorKind::Windsurf => &["windsurf"],
            EditorKind::Jetbrains => &[
                "rider", "idea", "pycharm", "webstorm", "phpstorm", "clion", "rubymine", "goland",
            ],
            EditorKind::Zed => &["zed"],
        }
    }
...
fn detect_visual_studio_uncached() -> Option<String> {
    ...
    install.get("displayName")?.as_str().map(|s| s.to_string())
}
...
mod tests {
    ...
    fn concurrent_callers_probe_once() {
        ...
        assert_eq!(
            ...
            1,
            "8 concurrent callers must probe once"
        );
        assert!(
    ...
    fn a_warm_cache_hit_does_not_wait_on_an_in_flight_probe() {
        ...
        assert_eq!(
            detect_once(&CACHE, &FLIGHT, || panic!("must not re-probe")),
            7
        );
    ...
    fn visual_studio_detection_agrees_with_vswhere() {
        ...
        if has_devenv {
            ...
            assert!(
                exe.exists(),
                "resolved Visual Studio exe does not exist: {exe:?}"
            );
    ...
    fn solution_scan_finds_nested_solutions_and_stops_at_depth() {
        ...
        assert!(
            !names.contains(&"TooDeep.sln"),
            "scan should stop at depth 2"
        );
        assert!(
            !names.contains(&"Ignored.sln"),
