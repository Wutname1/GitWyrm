...
pub async fn get_commit_stats(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        for sha in shas {
            ...
            if let Ok((files_changed, additions, deletions)) =
                cached_change_stats(&open, &repo, &commit)
            {
                out.insert(
                    sha,
                    CommitStats {
...
mod tests {
    ...
    fn rewound_head_hands_lane_zero_to_the_tip_above_it() {
        ...
        repo.reference("refs/remotes/origin/master", newer, true, "test remote")
            .expect("remote ref");
        let base_commit = repo.find_commit(base).expect("base commit");
    ...
    fn fetching_new_commits_invalidates_the_primary_lane_memo() {
        ...
        repo.reference("refs/remotes/origin/master", base, true, "test remote")
            .expect("remote ref");
        {
            let base_commit = repo.find_commit(base).expect("base commit");
            repo.reset(base_commit.as_object(), git2::ResetType::Hard, None)
        ...
        repo.reference("refs/remotes/origin/master", fetched, true, "fetched")
    ...
    fn collect_refs_labels_head_branches_and_remotes() {
        ...
        assert!(
            matches!(kind_of("master"), RefKind::Head),
            "checked-out branch"
        );
        assert!(matches!(kind_of("side"), RefKind::Branch), "other local");
        assert!(
            matches!(kind_of("origin/master"), RefKind::Remote),
            "remote"
        );
        assert!(
    ...
    fn primary_lane_cache_matches_a_fresh_computation_and_follows_head() {
        ...
        assert!(
            open.cached_primary_lane(newer, tips).is_none(),
            "starts empty"
        );
    ...
    fn diverged_tip_does_not_take_the_head_lane() {
        ...
        repo.reference("refs/heads/side", side, true, "test branch")
            .expect("side ref");
