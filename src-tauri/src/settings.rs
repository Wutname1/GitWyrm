...
/// mistakes.
fn migrate_telemetry(
    crash_reports: Option<bool>,
    usage_telemetry: Option<bool>,
) -> Option<TelemetryLevel> {
...
mod tests {
    ...
    fn forgetting_a_repo_clears_every_place_it_is_stored() {
        ...
        assert_eq!(
            settings.tab_order,
            vec!["group:g1".to_string(), kept.to_string()]
        );
    ...
    fn the_ai_switch_round_trips_through_settings_json() {
        for enabled in [false, true] {
            ...
            assert_eq!(
                back.ai_enabled, enabled,
                "ai_enabled should survive a round trip"
            );
        }
    }
    ...
    fn profiles_survive_a_write_and_reread() {
        ...
        assert_eq!(
            after.profiles.len(),
            1,
            "profile survived the frontend save"
        );
    ...
    fn tab_groups_round_trip_through_settings_json() {
        ...
        settings.repo_picker_collapsed_sections = vec!["recent".to_string(), "watched".to_string()];
        ...
        assert_eq!(
            restored.saved_tab_groups[0].repo_paths,
            vec!["C:\\code\\GitWyrm"]
        );
        assert_eq!(restored.pinned_repo_paths, vec!["C:\\code\\GitWyrm"]);
        assert_eq!(
            restored.pinned_saved_group_ids,
    ...
    fn stable_releases_default_to_reports_not_off() {
        for version in ["1.0.0", "2.4.1"] {
            ...
            assert!(
                level.reports_installs(),
                "install count must survive {version}"
            );
            assert!(
                level.reports_errors(),
                "crashes must still report at {version}"
            );
            assert!(
    ...
    fn an_unset_level_follows_the_build() {
        ...
        assert_eq!(
            telemetry_level_for(&settings, "0.4.0"),
            TelemetryLevel::Full
        );
