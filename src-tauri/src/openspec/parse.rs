...
pub fn parse_delta_file(capability: &str, file: &str, md: &str) -> Vec<SpecDelta> {
    ...
    for (heading, body) in sections(md, 2) {
        let Some(word) = heading.split_whitespace().next() else {
            continue;
        };
        let Some(kind) = DeltaKind::from_heading(word) else {
            continue;
        };
...
pub fn status_of(tasks: &[SpecTask], delta_count: usize) -> ChangeStatus {
    ...
    if done == tasks.len() {
        ...
        return if delta_count > 0 {
            ChangeStatus::ReadyToArchive
        } else {
    ...
    let build_open = tasks.iter().any(|t| !t.done && !is_review_group(&t.group));
    if build_open {
        ChangeStatus::InBuild
    } else {
...
mod tests {
    ...
    fn status_rules() {
        ...
        // Tasks but none done.
        assert_eq!(
            status_of(&[task("1. Build", false)], 1),
            ChangeStatus::Draft
        );
        // Partly built.
        assert_eq!(
        ...
        assert_eq!(
            status_of(&[task("1. Build", true)], 1),
            ChangeStatus::ReadyToArchive
        );
        // All done but nothing to merge: not archivable.
        assert_eq!(
            status_of(&[task("1. Build", true)], 0),
            ChangeStatus::InBuild
        );
    }
...
",
    ...
    fn missing_files_become_notes_not_failures() {
        ...
        assert!(parsed
            .notes
            .iter()
            .any(|n| n.contains("proposal.md is missing")));
    }
}
