...
pub fn check_ready(done: u32, total: u32, deltas: usize) -> Option<ArchiveBlock> {
    ...
    if done < total {
        return Some(ArchiveBlock::TasksOpen {
            open: total - done,
            total,
        });
    }
...
pub fn diagnose(change_id: &str, output: &str) -> ArchiveDiagnosis {
    ...
    if text.contains("already exists") && text.contains("archive") {
        let path = archive_path_from(output)
            .unwrap_or_else(|| format!("openspec/changes/archive/<dated>-{change_id}"));
    ...
    if text.contains("target spec does not exist") || text.contains("only added requirements") {
        ...
        return ArchiveDiagnosis {
            problem: ArchiveProblem::ModifiedInNewSpec {
                capability: capability.clone(),
            },
            summary: format!(
                "This change edits a requirement that {capability} does not have yet."
            ),
            detail: format!(
...
Aborted. No files were changed.";
    ...
    fn repair_removes_an_empty_leftover() {
        ...
        let leftover = dir
            .path()
            .join("openspec/changes/archive/2026-01-01-add-thing/specs/core");
