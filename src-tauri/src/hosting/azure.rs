...
use super::{
    ...
    IssueSummary, MergeMethod, PrDetail, PrSummary, RepoSlug,
};
...
impl AzureDevOps {
    ...
    fn request(
        ...
        let cred =
            http::credential(app, ProviderId::AzureDevops)?.ok_or_else(|| not_connected(HOST))?;
    ...
    async fn threads(
        ...
        let page: Page<ApiThread> = http::send_json(
            self.request(app, reqwest::Method::GET, url)?,
            HOST,
            ERROR_KEYS,
        )
        .await?;
        ...
        for thread in page.value {
            for comment in thread.comments {
                ...
                out.push(HostComment {
                    author_is_bot: false,
                    author: comment.author.map(|a| a.display_name).unwrap_or_default(),
...
impl HostProvider for AzureDevOps {
    ...
    async fn list_prs(
        ...
        let page: Page<ApiPr> = http::send_json(
            self.request(app, reqwest::Method::GET, url)?,
            HOST,
            ERROR_KEYS,
        )
    ...
    async fn comment(
        ...
        let created: ApiThread = http::send_json(
            self.request(app, reqwest::Method::POST, url)?
        ...
        Ok(HostComment {
            ...
            created_at: first.and_then(|c| c.published_date).unwrap_or_default(),
        })
    }
    ...
    async fn approve_pr(
        ...
        http::send(
            self.request(app, reqwest::Method::PUT, url)?
    ...
    async fn merge_pr(
        ...
        let url = self.git_url(app, &coord, &format!("/pullrequests/{number}"));
        http::send(
            self.request(app, reqwest::Method::PATCH, url)?
                .json(&serde_json::json!({
                  "status": "completed",
...
impl AzureDevOps {
    ...
    async fn profile_id(
        ...
        let data: ConnectionData = http::send_json(
            self.request(app, reqwest::Method::GET, url)?,
            HOST,
            ERROR_KEYS,
        )
