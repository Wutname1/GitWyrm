...
impl Bitbucket {
    fn request(
        ...
        let cred =
            http::credential(app, ProviderId::Bitbucket)?.ok_or_else(|| not_connected(HOST))?;
        ...
        Ok(match cred.email.as_deref() {
            ...
            Some(email) => {
                let encoded = base64::engine::general_purpose::STANDARD
                    .encode(format!("{email}:{}", cred.token));
    ...
    async fn comments(
        ...
        let path = format!(
            "{}/{kind}/{number}/comments?pagelen=100",
            self.repo_path(slug)
        );
        let page: Page<ApiComment> = http::send_json(
...
impl HostProvider for Bitbucket {
    ...
    async fn issue_detail(
        ...
        Ok(IssueDetail {
            ...
            labels: [issue.kind, issue.priority].into_iter().flatten().collect(),
