...
use super::driver::{GateAnswer, GateRequest, PreflightItem, RunDriver, RunState, RunStep};
...
impl RunDriver for ScriptedDriver {
        ...
        match answer {
            ...
            GateAnswer::FindAnotherWay => {
                ...
                self.beats.push_front(Beat::new(
                    RunStep::Adapted {
                        text: "Skipped that and wrote it by hand instead, with no new library."
                            .into(),
                    },
                    RunState::Working,
                ));
    ...
    fn note(&mut self, text: String) {
        ...
        self.beats
            .push_front(Beat::new(RunStep::YouSaid { text }, RunState::Working).after(0));
    }
...
fn script_for(scenario: Scenario) -> Vec<Beat> {
    let mut beats = vec![
        ...
        Beat::new(
            RunStep::Plan {
                text: "Add the toggle to the settings screen and wire it up.".into(),
            },
            RunState::Working,
        )
        .after(900),
    ...
    match scenario {
        ...
        Scenario::Gate => {
            beats.push(
                Beat::new(
                    RunStep::Gate {
                        request: GateRequest::AddDependency { name: "zod".into() },
                    },
                    RunState::NeedsYou,
                )
                .after(800),
...
mod tests {
    ...
    fn a_note_is_queued_and_echoed_without_interrupting() {
        ...
        assert!(
            matches!(next.step, RunStep::YouSaid { .. }),
            "the note is echoed back"
        );
    }
    ...
    fn preflight_says_when_there_is_nothing_to_read() {
        let RunStep::Preflight { items } = preflight() else {
            panic!()
        };
        let none = items
            .iter()
            .find(|i| !i.done)
            .expect("one item honestly has nothing to do");
