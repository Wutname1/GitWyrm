...
pub async fn run_task(
  ...
  let (state, detail) = match outcome {
    Ok(stop) => match stop {
      StopReason::EndTurn => (RunState::Finished, "Finished. Your changes are ready to look over.".to_string()),
...
fn handle(item: Incoming, sink: &Sink, answers: &mpsc::Receiver<GateAnswer>) {
  match item {
    ...
    Incoming::PermissionRequest { tool_call, options, respond } => {
...
fn pick(options: &[crate::ai::agent::acp::PermissionOption], allow: bool) -> PermissionDecision {
  ...
  match chosen {
    Some(o) if allow => PermissionDecision::AllowOnce { option_id: o.option_id.clone() },
    Some(o) => PermissionDecision::RejectOnce { option_id: o.option_id.clone() },
...
fn describe(tool_call: &serde_json::Value) -> GateRequest {
  ...
  GateRequest::Unclassified { summary: title.to_string() }
}
...
mod tests {
  ...
  fn opt(kind: &str, id: &str) -> PermissionOption {
    PermissionOption { option_id: id.into(), name: kind.into(), kind: kind.into() }
  }
  ...
  fn nothing_usable_cancels_rather_than_guessing() {
    ...
    assert!(matches!(pick(&options, true), PermissionDecision::Cancelled));
  }
