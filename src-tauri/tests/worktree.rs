...
fn list_includes_the_main_checkout_first() {
  let Some((root, repo_path)) = fixture("list") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  ...
  assert!(list[0].is_current, "the handle we asked with is the current row");
...
fn add_can_create_the_branch_at_the_same_time() {
  let Some((root, repo_path)) = fixture("addb") else { return };
  let wt_path = root.join("project-new");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "brand-new", true, Some("main"))
    .unwrap();

  let repo = Repository::open(&repo_path).unwrap();
  assert!(repo.find_branch("brand-new", git2::BranchType::Local).is_ok());
  let list = worktree::list(&repo, None);
  assert!(list.iter().any(|w| w.branch.as_deref() == Some("brand-new")));
...
fn adding_a_branch_already_checked_out_is_refused_in_plain_words() {
  let Some((root, repo_path)) = fixture("collide") else { return };
  let first = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&first), "feature", false, None).unwrap();

  let second = root.join("project-feature-again");
  let err = worktree::add(&path_str(&repo_path), &path_str(&second), "feature", false, None)
    .unwrap_err()
    .to_string();
  assert!(
...
fn holder_of_branch_names_the_folder_that_has_it() {
  let Some((root, repo_path)) = fixture("holder") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
...
fn dirty_count_keeps_modified_and_untracked_apart() {
  let Some((root, repo_path)) = fixture("dirty") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
...
fn remove_refuses_dirty_with_the_counts_the_confirm_needs() {
  let Some((root, repo_path)) = fixture("refuse") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  fs::write(wt_path.join("README.md"), "# changed").unwrap();
...
fn remove_leaves_the_branch_and_reports_whether_it_is_merged() {
  let Some((root, repo_path)) = fixture("removeclean") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  ...
  match outcome {
    RemoveOutcome::Removed { branch, branch_merged } => {
...
fn a_branch_with_its_own_commits_is_not_reported_merged() {
  let Some((root, repo_path)) = fixture("unmerged") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  ...
  git(&wt_path, &["add", "."]);
  git(&wt_path, &["commit", "-qm", "work that exists nowhere else"]);
...
fn keeping_the_changes_stashes_them_including_untracked() {
  let Some((root, repo_path)) = fixture("keep") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  identity(&wt_path);
  fs::write(wt_path.join("README.md"), "# changed").unwrap();
...
fn discarding_removes_a_dirty_worktree() {
  let Some((root, repo_path)) = fixture("discard") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  fs::write(wt_path.join("README.md"), "# changed").unwrap();
...
fn remove_refuses_the_main_checkout() {
  let Some((root, repo_path)) = fixture("main") else { return };
  let repo = Repository::open(&repo_path).unwrap();
...
fn a_deleted_folder_reads_as_missing_and_prunes() {
  let Some((root, repo_path)) = fixture("missing") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  ...
  let repo = Repository::open(&repo_path).unwrap();
  let list = worktree::list(&repo, None);
  let broken = list.iter().find(|w| !w.is_main).expect("the row survives the folder");
  assert_eq!(broken.state, WorktreeState::Missing);

  let pruned = worktree::prune(&repo).unwrap();
  assert_eq!(pruned, 1);
  assert_eq!(worktree::list(&repo, None).len(), 1, "only the main checkout is left");
...
fn a_moved_folder_reads_as_moved_and_repairs() {
  let Some((root, repo_path)) = fixture("moved") else { return };
  let wt_path = root.join("project-feature");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "feature", false, None).unwrap();
  ...
  let repo = Repository::open(&repo_path).unwrap();
  let list = worktree::list(&repo, None);
  let broken = list.iter().find(|w| !w.is_main).expect("the row survives the move");
  assert_eq!(
    broken.state,
  ...
  assert!(repaired.path.replace('\\', "/").ends_with("project-feature-moved"));
...
fn ignored_survey_offers_env_files_but_not_dependency_folders() {
  let Some((root, repo_path)) = fixture("ignored") else { return };
  ...
  fs::write(repo_path.join("node_modules/react/index.js"), "module.exports={}").unwrap();
...
fn suggested_path_is_outside_the_repository() {
  let Some((root, repo_path)) = fixture("suggest") else { return };
...
fn run_worktrees_are_marked_without_touching_the_working_folder() {
  let Some((root, repo_path)) = fixture("runmark") else { return };
  let wt_path = root.join("project-run");
  worktree::add(&path_str(&repo_path), &path_str(&wt_path), "run-branch", true, Some("main"))
    .unwrap();
