...
fn auto_stash_carries_nonconflicting_change() {
    ...
    // auto-stash sequence
    repo.stash_save(&sig(), "auto", Some(StashFlags::INCLUDE_UNTRACKED))
        .unwrap();
    ...
    assert!(
        repo.stash_pop(0, None).is_err(),
        "no stash should remain after a clean pop"
    );
    let _ = fs::remove_dir_all(&dir);
}
...
fn carry_succeeds_when_no_conflict() {
    ...
    assert!(
        is_dirty(&repo),
        "changes should still be present after carry"
    );
    let _ = fs::remove_dir_all(&dir);
}
...
fn auto_stash_pop_conflict_keeps_stash() {
    ...
    assert!(
        ...
        "apply should conflict on the shared line"
    );
    ...
    assert_eq!(
        count, 1,
        "stash must be kept as a backup after a conflicting apply"
    );
    let _ = fs::remove_dir_all(&dir);
}
...
fn git2_stash_pop_drops_stash_on_conflict() {
    ...
    assert_eq!(
        count, 0,
        "git2 stash_pop drops the stash on conflict (the pitfall)"
    );
    let _ = fs::remove_dir_all(&dir);
}
...
fn add_remote_ref(repo: &Repository, full: &str, oid: git2::Oid) {
    repo.reference(&format!("refs/remotes/{full}"), oid, true, "test")
        .unwrap();
}
...
fn raw_remote_ref_checkout_detaches_head() {
    ...
    assert!(
        ...
        "raw remote checkout detaches HEAD"
    );
    let _ = fs::remove_dir_all(&dir);
}
...
fn remote_branch_resolves_to_new_tracking_branch() {
    ...
    let upstream = branch
        .upstream()
        .expect("upstream should be set for a configured remote");
...
fn remote_branch_with_slashes_keeps_full_short_name() {
    ...
    assert_eq!(
        target, "feature/nested",
        "only the remote prefix is stripped"
    );
    switch_to(&repo, &target).unwrap();
...
fn local_and_unknown_names_pass_through() {
    ...
    assert_eq!(
        resolve_switch_target(&repo, "nope/missing").unwrap(),
        "nope/missing"
    );
    let _ = fs::remove_dir_all(&dir);
}
