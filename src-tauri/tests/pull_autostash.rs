...
fn pull_succeeds_with_conflicting_dirty_file() {
  ...
  assert!(!plain_ok, "expected a plain merge to refuse a dirty tree, but it succeeded");
  assert!(
  ...
  assert!(ok, "pull with --autostash should succeed on a dirty tree: {text}");
  ...
  assert!(content.contains("my local work"), "local edit must survive the pull, got: {content}");

  // And the remote commit actually landed.
  let log = git_ok(&clone, &["log", "--oneline"]);
  assert!(log.contains("remote edit"), "remote commit should be present: {log}");
}
...
fn pull_succeeds_with_unrelated_dirty_file() {
  ...
  assert!(content.contains("unrelated local work"), "unrelated edit must survive: {content}");
}
...
fn pull_preserves_staged_changes() {
  ...
  assert!(status.contains("new.txt"), "staged file should still be present: {status}");
  assert!(fs::read_to_string(clone.join("new.txt")).unwrap().contains("staged work"));
}
...
fn pull_on_clean_tree_creates_no_stash() {
  ...
  assert!(stashes.trim().is_empty(), "clean pull must not leave a stash: {stashes}");
}
...
fn rebase_succeeds_with_dirty_tree() {
  ...
  assert!(dirty.contains("in progress"), "dirty file must survive rebase: {dirty}");

  // And the rebase genuinely replayed onto main's commit.
  let log = git_ok(&clone, &["log", "--oneline"]);
  assert!(log.contains("feature commit"), "feature commit missing: {log}");
  assert!(log.contains("main commit"), "should now sit on top of main: {log}");
}
