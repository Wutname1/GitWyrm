...
fn push_of_new_commits_is_observed_by_a_long_lived_handle() {
    ...
    assert_eq!(
        ahead_behind(&repo),
        (0, 0),
        "fresh checkout should be in sync"
    );
    ...
    assert_eq!(
        after, 0,
        "handle must see the pushed ref, not a cached value"
    );
...
fn up_to_date_push_reports_nothing_moved() {
    ...
    assert_eq!(
        ...
        0,
        "no-op push must report zero commits"
    );
...
fn push_of_a_branch_that_is_not_checked_out_reports_its_own_count() {
    ...
    assert_eq!(
        before, 2,
        "feature should read as ahead by 2 while main is checked out"
    );
    ...
    git(
        &work,
        &["push", "-q", "--progress", "origin", "refs/heads/feature"],
    );
...
fn publishing_a_never_pushed_branch_sets_upstream_and_counts_its_commits() {
    ...
    assert!(
        branch_ahead_behind(&repo, "fresh").is_none(),
        "fresh should have no upstream"
    );
    ...
    assert_eq!(
        published_count(&repo, "fresh"),
        3,
        "three commits went to the remote"
    );
...
fn diverged_branch_refuses_to_fast_forward() {
    ...
    assert!(
        !status.success(),
        "a non-fast-forward update must be rejected, not silently applied"
    );
...
fn pull_reports_commits_actually_received() {
    ...
    assert_eq!(
        behind_before - behind_after,
        1,
        "pull should report 1 commit received"
    );
