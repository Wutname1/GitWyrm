...
fn scratch_repo(tag: &str) -> (PathBuf, Repository) {
    let dir =
        std::env::temp_dir().join(format!("gitwyrm-cleanapply-{}-{}", tag, std::process::id()));
...
fn clean_cherry_pick_leaves_no_operation_state() {
    ...
    assert!(
        !state.merging,
        "a committed pick must not report an operation"
    );
...
fn overlapping_but_clean_pick_leaves_no_operation_state() {
    ...
    let merged = fs::read_to_string(dir.join("a.txt"))
        .unwrap()
        .replace("\r\n", "\n");
    assert_eq!(
        merged, "CHANGED1\nl2\nl3\nl4\nl5\nl6\nl7\nl8\nCHANGED9\n",
...
fn clean_merge_commits_itself() {
    ...
    assert!(
        !state.merging,
        "a clean merge must not wait to be committed"
    );
    assert_eq!(repo.state(), git2::RepositoryState::Clean);
