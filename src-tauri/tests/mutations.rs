...
fn scratch_repo_named(label: &str) -> (PathBuf, Repository) {
    let dir = std::env::temp_dir().join(format!("gitwyrm-test-{}-{label}", std::process::id()));
...
fn full_stage_commit_stash_branch_cycle() {
    ...
    let second = {
        ...
        repo.commit(Some("HEAD"), &sig(), &sig(), "add code", &tree, &[&parent])
            .unwrap()
    };
...
fn tag_and_branch_delete_cycle() {
    ...
    repo.tag("v0.2.0", &head, &sig(), "second release", false)
        .unwrap();
    ...
    let mut feature = repo
        .find_branch("feature/y", git2::BranchType::Local)
        .unwrap();
...
fn rename_branch_moves_the_ref_and_refuses_an_existing_name() {
    ...
    let mut again = repo
        ...
        .unwrap();
    assert!(
        again.rename("taken", false).is_err(),
        "rename must not overwrite an existing branch"
    );
...
fn ref_name_validation_matches_git_rules() {
    ...
    assert!(
        !git2::Reference::is_valid_name("main"),
        "shorthand is not a valid refname"
    );
    assert!(
        git2::Reference::is_valid_name("refs/heads/@"),
        "a lone @ is legal once prefixed"
    );
