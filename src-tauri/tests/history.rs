...
fn scratch_repo(label: &str) -> (PathBuf, Repository) {
  let dir = std::env::temp_dir().join(format!("gitwyrm-test-history-{}-{label}", std::process::id()));
...
fn log_summaries(repo: &Repository) -> Vec<String> {
  ...
  walk.push(repo.head().unwrap().peel_to_commit().unwrap().id()).unwrap();
  walk
    .map(|oid| {
      repo
...
fn squash_non_contiguous_moves_selection_together() {
  ...
  assert_eq!(log_summaries(&repo), vec!["add b", "add a and c", "initial"]);
...
fn conflicting_rewrite_aborts_and_restores() {
  ...
  assert!(err.to_string().contains("nothing was changed"), "got: {err}");
