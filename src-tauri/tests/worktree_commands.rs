...
fn the_branch_that_will_not_delete_resolves_in_one_flow() {
    let Some((root, repo_path)) = fixture("chain") else {
        return;
    };
    ...
    branch
        .delete()
        .expect("the branch deletes once nothing holds it");
...
fn a_dirty_worktree_is_refused_then_removed_by_choice() {
    let Some((root, repo_path)) = fixture("dirtychain") else {
        return;
    };
    let wt_path = root.join("project-feature");
    worktree::add(&s(&repo_path), &s(&wt_path), "feature", false, None).unwrap();
...
fn a_folder_deleted_by_hand_prunes_cleanly() {
    let Some((root, repo_path)) = fixture("handdel") else {
        return;
    };
    let wt_path = root.join("project-feature");
    worktree::add(&s(&repo_path), &s(&wt_path), "feature", false, None).unwrap();
...
fn moving_the_whole_project_is_repaired_in_one_step() {
    let Some((root, repo_path)) = fixture("moverepo") else {
        return;
    };
...
fn a_worktree_made_in_a_terminal_is_listed_and_removable() {
    let Some((root, repo_path)) = fixture("external") else {
        return;
    };
...
fn committing_in_a_worktree_leaves_the_main_checkout_untouched() {
    let Some((root, repo_path)) = fixture("isolate") else {
        return;
    };
    let wt_path = root.join("project-feature");
    worktree::add(&s(&repo_path), &s(&wt_path), "feature", false, None).unwrap();
    ...
    git(
        &wt_path,
        &["commit", "-qm", "work done in the other folder"],
    );
...
fn suggestions_never_land_inside_the_project() {
    let Some((root, repo_path)) = fixture("suggestsafe") else {
        return;
    };
    for branch in [
        "feature/login",
        ...
        assert!(
            worktree::path_problem(&repo_path, p).is_none(),
            "{suggested} should be usable"
        );
    }
    let _ = fs::remove_dir_all(&root);
}
