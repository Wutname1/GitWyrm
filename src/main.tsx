...
// Reporting is opt-out, and honouring that means asking before the reporter
// starts. This read runs ahead of the first render rather than letting the app
// boot un-gated and switching the reporter on late -- that window is exactly
// when startup crashes happen, and reporting one from a user who opted out is
...
//
// The level is resolved by the backend rather than read off the settings field,
// because "unset" is not a level -- the default depends on the version and
// update channel, and that rule lives in Rust so there is only one copy of it.
try {
  const level = await commands.getTelemetryLevel()
  initSentry(level.status === 'ok' ? level.data : 'reports')
} catch {
  // Crashes still report; diagnostics do not, since an unknown answer should
  // not be read as consent to the widest level.
  initSentry('reports')
}
...
// silently blanking the window. Mirror them into the app log so a bug report
