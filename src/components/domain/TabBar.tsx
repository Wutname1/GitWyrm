...
function RecentRepositories({ compact = false }: { compact?: boolean }) {
  ...
  const recentMatches = recents.filter(
    (repo) =>
      !openPaths.has(repo.path.toLowerCase()) && matches(repo.name, repo.path),
  );
              ...
              <RepoRow
                ...
                icon={
                  <FolderOpen
                    ...
                    strokeWidth={2}
                    className={
                      repo.id === activeRepoId ? "text-accent-text" : undefined
                    }
                  />
                }
...
export function VerticalTabRail() {
  ...
  const setVerticalTabWidth = useWorkspaceStore(
    (state) => state.setVerticalTabWidth,
  );
  const showSettings = useUiStore((state) => state.showSettings);
    ...
    >
      ...
      <div
        ...
        onKeyDown={(event) => {
          ...
          setVerticalTabWidth(
            verticalTabWidth + (event.key === "ArrowLeft" ? -8 : 8),
          );
        }}
        className={cn(
