...
- [ ] 1.1 `git/worktree.rs`: a `Worktree` type carrying folder name, absolute path,
...
- [ ] 1.2 `list`: git2 `Repository::worktrees` + `find_worktree`, with the main
...
- [ ] 1.3 Broken detection as a typed outcome, not an error string: distinguish
...
- [ ] 1.4 `add`: shell `git worktree add`, with `-b` when creating a branch. Validates
...
- [ ] 1.5 `remove`: refuses the currently open worktree and the main checkout; reports
...
- [ ] 1.6 `prune` (git2 `worktree_prune`) and `repair` (shell `git worktree repair
...
- [ ] 1.7 `dirty_count(path)`: modified and untracked counts kept separate, for a
      ...
      no way back.
- [ ] 1.8 Typed removal outcomes: `removed`, `refused_dirty { modified, untracked }`,
...
- [ ] 1.9 Keep-the-changes path: set a dirty worktree's changes aside recoverably before
      removing it, reusing the existing stash plumbing rather than inventing a second
      way to park work.
- [ ] 1.10 `branch_delete` gains a typed refusal naming the worktree that holds the
      branch, so the branch-delete prompt can offer to remove it.
- [ ] 1.11 Ignored-file survey for a new worktree: list ignored files worth copying,
...
- [ ] 2.1 `commands/worktree.rs` exposing list / add / remove / prune / repair /
      dirty_count over the `git/worktree.rs` layer. Regenerate `bindings.ts` with the
      bindings script - never hand-edit it.
- [ ] 2.2 Branch-checked-out-elsewhere detection returning *which* worktree holds the
...
- [ ] 2.3 Default-path suggestion: `../<repo-name>-<branch-slug>`, de-duplicated if
      that folder already exists. Computed in Rust so the modal and any future caller
      agree on it.
- [ ] 2.4 File watcher covers open worktrees, and watches the repository's worktree
...
- [ ] 2.5 Release before remove: stop watching the worktree path and drop any open
...
- [ ] 2.6 Unit tests over a temp repo: add, list (including main checkout), branch
...
- [ ] 3.1 `WorktreesSection` in `left-panel/`, modeled on `SubmodulesSection`: rows
      with folder name, branch, an open marker, and a per-row context menu.
- [ ] 3.2 Three visibility states, not two: setting off and none exist -> no section;
...
- [ ] 3.3 Empty state (setting on, none exist): a plain-language line saying what a
      worktree is, plus Add. The section is present before it is needed - that is how
      the feature gets found.
- [ ] 3.4 Auto-enable is a first-encounter courtesy only: it fires when the user has
      ...
      worktrees visible.
- [ ] 3.5 Broken rows: `missing` shows Prune, `moved` shows Repair, each with a
      one-line explanation of what happened. Repair asks where the folder went.
- [ ] 3.6 `useWorktrees` query in `useGitQueries.ts` and mutations in
...
- [ ] 3.7 An open worktree tab whose folder disappears externally reports that plainly
      rather than showing stale contents.

## 4. UI: add, open, remove

- [ ] 4.1 `AddWorktreeModal`: branch picker (existing or new, saying which commit a
...
- [ ] 4.2 Open a worktree as its own repository tab via the existing open-by-path
      path. If it is already open, switch to that tab rather than opening a second.
- [ ] 4.3 Tab title and subtitle disambiguate sibling checkouts of one repository
      (folder name + branch), so two tabs of the same repo are never identical.
- [ ] 4.4 Status bar names the active worktree when the open checkout is not the main
      one.
- [ ] 4.5 Remove confirm via `ConfirmDialog`: states modified and untracked counts (or
...
- [ ] 4.6 Branch-taken explanation surfaces wherever a checkout starts: names the
      worktree holding it and offers Open that worktree.
- [ ] 4.7 Branch-delete prompt handles the held-by-worktree refusal: names the worktree,
...
- [ ] 4.8 Locked-folder failure state: says a program is still using the folder, suggests
...
- [ ] 4.9 After a successful removal, offer to delete the branch when it is merged;
      say the work is still on it and offer nothing when it is not.
- [ ] 4.10 Create-time copy offer in `AddWorktreeModal`: lists ignored files found worth
...
- [ ] 5.1 "Work in its own folder so you can keep editing" option on single-run start,
      unticked by default. Provisioning goes through the same path the agent room
      uses; the room never asks.
- [ ] 5.2 Run working-directory plumbing: the run's tools operate in the worktree path
      rather than the repository path, and the guardrails' in-repo path check accepts
      that folder as the run's repository.
- [ ] 5.3 Guardrail line names the folder whenever the run is not working in the user's
...
- [ ] 5.4 Run worktrees are listed in the Worktrees section while they exist, marked as
      belonging to a run.
- [ ] 5.5 Completed isolated run is reviewed as a diff before anything reaches the
      user's branch.
- [ ] 5.6 Discard: remove the worktree when it holds only what the run wrote; when
